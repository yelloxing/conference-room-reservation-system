 ...
 *
 * // 方法三：获取错误信息
 * let error=ERROR.getError(target);
 */
    ...
    install(Vue) {
        Vue.prototype.$error = function (formname) {
            ...
            if (xhtml.isNode(formname)) {
                ...
                // 获取需要校验的表单结点
                let temp = this.$el.getElementsByTagName('form');
            ...
            return {
                ...
                // 获取第一个错误的输入框信息
                "first": function () {
                    let nodes = formnode.getElementsByTagName('*'),
                        target;
                ...
                "getError": function (target) {
                    ...
                    if (!error) {
                        ...
                        let inputName = target.getAttribute('validate-name') || target.getAttribute('name');

                        // 设置默认的未自定义错误提示方式的默认提示
                        error = inputName + "输入非法！";
