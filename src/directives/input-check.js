...
let doIt = function (el, binding) {
    ...
    // 传递的规则
    let ruls = binding.value.length > 1 ? binding.value[1].split("#") : [],
        rul, rulName, temp;
    ...
    for (let i = 0; i < ruls.length; i++) {
        ...
        // 针对特殊的选项，进行取值重定向
        if (el.getAttribute('deftype') === 'select') {
            let select_helper = el.nextElementSibling;
            while (xhtml.isNode(select_helper) && select_helper.getAttribute('deftype') !== 'show-helper') {
                select_helper = select_helper.nextElementSibling;
            }

            if (select_helper.getAttribute('deftype') === 'show-helper') {
                rul[0] = select_helper.innerText;
            } else {
                // todo
            }
        }
        ...
        // 求解是否满足规则
        temp = (validate[rulName] || function () { return true; })(el, ...rul);
